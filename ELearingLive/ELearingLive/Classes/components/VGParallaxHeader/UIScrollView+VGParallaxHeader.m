...
@property (nonatomic, readwrite) CGFloat updateHeight;
...
{
    objc_setAssociatedObject(self, &UIScrollViewScrollCurrentViewDidScroll, scrollCurrentViewDidScroll, OBJC_ASSOCIATION_COPY);
}

- (void(^)(CGFloat contentOffset))scrollCurrentViewDidScroll
{
    return objc_getAssociatedObject(self, &UIScrollViewScrollCurrentViewDidScroll);
}
                       ...
                       height:(CGFloat)height
{
    ...
    self.parallaxHeader.updateHeight = height;
...
{
    self.parallaxHeader.updateHeight = height;
    self.parallaxHeader.originalHeight = height;
...
- (void)positionScrollViewParallaxHeader
{
    ...
    if (self.contentOffset.y < 0) {
        ...
        if (self.parallaxHeader.containerView && self.parallaxHeader.containerView.subviews.count > 0) {
            UIView *iv = self.parallaxHeader.containerView.subviews[0];
                       ...
                       context:(void *)context
{
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(contentInset))] && context == VGParallaxHeaderObserverContext) {
        ...
        if(!self.isInsideTableView) {
            /**
             *  重新设置 contentOffset, view 会滑动到顶部
             */
