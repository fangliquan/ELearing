...
#define SYSTEM_DATABASE_NAME_PREFIX @"gdmall_system"
#define USER_DATABASE_NAME_PREFIX @"gdmall_user_v2"

#define OLD_USER_DATABASE_NAME_PREFIX @"gdmall_user_v1"
...
-(void)generateDBDefine
{
    ...
    // user db
    NSArray *userDBTables = [[NSArray alloc] initWithObjects:[UserLoginResponse class],[GoodsShoppingOrder class],
                                nil];
        ...
        for (int i = 0; i < [dbTables count]; i++){
            ...
            if (![self isTable:[tableClass getTableName] inArray:tables])
            {
                ...
                if ([[tableClass getTableName] isEqualToString:[Informations getTableName]]) {
                    ...
                    dbCreateTime.itemValue = [NSString stringWithFormat:@"%f", [DateHelper timeIntervalNow]];
...
-(BOOL)prepareSystemDB
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@_%@.db", DEFAULT_ADDRESS, SYSTEM_DATABASE_NAME_PREFIX];
...
-(BOOL)prepareUserDB
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@_%@_%ld.db", DEFAULT_ADDRESS, USER_DATABASE_NAME_PREFIX, userId];
...
- (BOOL)deleteCurrentUserDB {
    BOOL ret = [self deleteUserDB:USER_DATABASE_NAME_PREFIX andUserId:[CloudManager sharedInstance].currentAccount.loginInfo.userId];
    if (ret) {
